...
export default function VoiceRoomFixed({ roomId, userName, onLeave }: Props) {
  ...
  const flushCandidates = useCallback(async (id: string, pc: RTCPeerConnection) => {
    ...
    for (const candidate of queued) {
      try { await pc.addIceCandidate(candidate); log('added queued ICE', id); }
      catch (e) { console.warn('[WebRTC] queued ICE rejected', id, e); }
    }
  }, [log]);
  ...
  const createPeer = useCallback((remote: Participant) => {
    const existing = peers.current.get(remote.id);
    ...
    log('create peer', remote.id, 'initiator=', selfRef.current ? initiates(selfRef.current, remote) : false);
    for (const track of stream.getAudioTracks()) pc.addTrack(track, stream);

    pc.onicecandidate = ({ candidate }) => {
      if (!candidate || peers.current.get(remote.id) !== pc) return;
      log('sending ICE', remote.id, candidate.candidate);
    ...
    pc.ontrack = ({ streams, track }) => {
      ...
      log('ontrack', remote.id, track.kind);
    ...
    pc.oniceconnectionstatechange = () => {
      if (peers.current.get(remote.id) !== pc) return;
      log('ICE state', remote.id, pc.iceConnectionState);
      if (pc.iceConnectionState === 'connected' || pc.iceConnectionState === 'completed') setStatus(remote.id, 'connected');
        ...
        if (selfRef.current && initiates(selfRef.current, remote)) reconnectRef.current(remote.id, remote);
      }
    };

    pc.onconnectionstatechange = () => {
      if (peers.current.get(remote.id) !== pc) return;
      log('connection state', remote.id, pc.connectionState);
      if (pc.connectionState === 'connected') setStatus(remote.id, 'connected');
      else if (pc.connectionState === 'connecting') setStatus(remote.id, 'connecting');
      else if (pc.connectionState === 'disconnected') setStatus(remote.id, 'disconnected');
    ...
    pc.onsignalingstatechange = () => {
      if (peers.current.get(remote.id) === pc) log('signaling state', remote.id, pc.signalingState);
    };
    return pc;
  ...
  const makeOffer = useCallback(async (remote: Participant, iceRestart = false) => {
    ...
    try {
      log('create offer', remote.id, iceRestart);
      const offer = await pc.createOffer({ iceRestart });
      if (peers.current.get(remote.id) !== pc) return;
      await pc.setLocalDescription(offer);
      if (!pc.localDescription || peers.current.get(remote.id) !== pc) return;
      log('sending offer', remote.id);
      await signal({ type: 'sdp-offer', from: local.id, to: remote.id, sdp: pc.localDescription }, remote.id);
    } catch (e) {
      console.error('[WebRTC] offer error', remote.id, e);
      destroyPeer(remote.id, pc);
    } finally {
      offerLocks.current.delete(remote.id);
    }
  }, [createPeer, destroyPeer, log, signal]);
  ...
  const scheduleReconnect = useCallback((id: string, remote: Participant) => {
    ...
    const t = window.setTimeout(() => {
      ...
      if (stopped.current || !selfRef.current || !initiates(selfRef.current, remote)) return;
      destroyPeer(id);
  ...
  const handleEnvelope = useCallback(async (item: SignalEnvelope) => {
    ...
    if (payload.type === 'join') {
      ...
      updateMembers(membersRef.current.some(x => x.id === p.id) ? membersRef.current.map(x => x.id === p.id ? p : x) : [...membersRef.current, p]);
      if (initiates(local, p)) void makeOfferRef.current(p);
      return;
    }
    ...
    if (payload.type === 'chat-message') {
      setMessages(m => m.some(x => x.id === payload.id) ? m : [...m.slice(-99), { id: payload.id, from: payload.from, senderName: payload.senderName, text: payload.text, timestamp: payload.timestamp }]);
      return;
    }
    ...
    if (!remote) {
      ...
      q.push(item);
      pendingSignals.current.set(item.from, q.slice(-50));
      return;
    }
    ...
    if (payload.type === 'ice-candidate') {
      ...
      if (pc.remoteDescription) {
        try { log('received ICE', item.from); await pc.addIceCandidate(payload.candidate); }
        catch (e) { console.warn('[WebRTC] ICE error', item.from, e); }
      } else {
        const q = candidateQueue.current.get(item.from) || [];
        if (!q.some(c => JSON.stringify(c) === JSON.stringify(payload.candidate))) q.push(payload.candidate);
        candidateQueue.current.set(item.from, q);
        log('queue ICE', item.from, q.length);
      }
      return;
    }

    if (payload.type === 'sdp-offer') {
      if (initiates(local, remote)) { log('ignored offer from non-initiator', item.from); return; }
      let pc = peers.current.get(item.from);
      if (!pc || pc.signalingState === 'closed') pc = createPeer(remote);
      try {
        log('received offer', item.from);
        ...
        log('sending answer', item.from);
        await signal({ type: 'sdp-answer', from: local.id, to: item.from, sdp: pc.localDescription }, item.from);
      } catch (e) {
        console.error('[WebRTC] answer error', item.from, e);
        destroyPeer(item.from, pc);
      }
      return;
    }

    if (payload.type === 'sdp-answer') {
      const pc = peers.current.get(item.from);
      if (!pc || pc.signalingState !== 'have-local-offer') { log('ignored stale/duplicate answer', item.from); return; }
      try {
        log('received answer', item.from);
        await pc.setRemoteDescription(payload.sdp);
        await flushCandidates(item.from, pc);
      } catch (e) {
        console.error('[WebRTC] remote answer error', item.from, e);
        destroyPeer(item.from, pc);
      }
    }
  }, [createPeer, destroyPeer, flushCandidates, log, signal, updateMembers]);
  ...
  const processPending = useCallback(async (id: string) => {
    ...
    for (const item of items) await handleEnvelope(item);
  }, [handleEnvelope]);
  ...
  const heartbeat = useCallback(async () => {
    ...
    try {
      const r = await fetch('/api/room/heartbeat', {
        method: 'POST', headers: { 'Content-Type': 'application/json' }, cache: 'no-store',
        body: JSON.stringify({ roomId, participantId: local.id, lastSignalTimestamp: lastSignal.current, updates: { isMuted: mutedRef.current, isDeafened: deafenedRef.current } }),
      });
      if (!r.ok) throw new Error(`heartbeat ${r.status}`);
      const data = await r.json() as { signals?: SignalEnvelope[]; currentMembers?: Participant[] };
      // The member snapshot MUST be installed before processing signals. An offer can arrive in the same
      // polling response as the member list; the old code processed the offer first and dropped it forever.
      ...
      for (const item of signals) {
        ...
        try {
          await handleEnvelope(item);
          seenSignals.current.add(item.id);
          maxTimestamp = Math.max(maxTimestamp, item.timestamp);
        } catch (e) {
          console.error('[WebRTC] signal processing failed; retrying', item.id, e);
          break;
        }
      }
      lastSignal.current = Math.max(lastSignal.current, maxTimestamp);
      ...
      for (const remote of next) {
        ...
        if (!pc || pc.signalingState === 'closed') void makeOfferRef.current(remote);
      }
    } catch (e) {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      reconnectTimers.current.forEach(t => clearTimeout(t)); reconnectTimers.current.clear();
